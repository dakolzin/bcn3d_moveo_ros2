...
class JointStateConverter(Node):
    def __init__(self):
        ...
        # Установите последовательное соединение с Arduino
    ...
    def joint_state_callback(self, msg):
        ...
        # Проверка, совпадают ли полученные данные с последними отправленными
        if data_received == self.last_sent_data:
            self.retry_count = 0

        # Решение о том, нужно ли отправлять данные
        if data_to_send != self.last_sent_data or self.retry_count < 2:
            send_data = True
        else:
            send_data = False
