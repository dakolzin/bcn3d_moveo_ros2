...
class JointStateConverter(Node):
    def __init__(self):
        ...
        self.joint_state_sub = self.create_subscription(JointState, 'fsr_joint_states', self.joint_state_callback, 10)
    ...
    def handle_data_sending(self, data_to_send):
        ...
        if should_send_data:
            ...
            self.get_logger().debug(f"Data not sent (matches previous or exceeded retry limit): {data_to_send.strip()}")  
    ...
    def send_data_to_arduino(self, data_to_send):
        ...
        self.get_logger().debug(f"Sending data to Arduino: {data_to_send.strip()}")  
        self.get_logger().debug(f"===============================================")
        ...
        if current_time - self.last_print_time >= 1.0:  
            frequency = self.message_count / (current_time - self.last_print_time)
            self.get_logger().info(f'Частота отправки данных: {frequency:.2f} сообщений/сек')
